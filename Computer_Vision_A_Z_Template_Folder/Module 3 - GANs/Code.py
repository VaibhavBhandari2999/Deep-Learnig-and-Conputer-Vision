...
transform = transforms.Compose([transforms.Resize(imageSize), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),]) # We create a list of transformations (scaling, tensor conversion, normalization) to apply to the input images. ToTenser() converts a PIL Image or numpy.ndarray to tensor. Normalize basically Normalize a tensor image with mean and standard deviation.  As transorms.scale is deprecated, we use transforms.Resize
...
if __name__ == "__main__":  
    for epoch in range(25): # We iterate over 25 epochs. We can take more or less, but 25 gives a good value
        ...
        # 1st Step: Updating the weights of the neural network of the discriminator        
            ...
            output = netD(input) # We forward propagate this real image into the neural network of the discriminator to get the prediction (a value between 0 and 1). The input is a torch variable. For each image, output is anumber between 0 and 1, 0 closer to rejection, 1 closer to accepting
            errD_real = criterion(output, target) # We compute the loss between the predictions (output) and the target (equal to 1).
            
            # Training the discriminator with a fake image generated by the generator. Same as before
            noise = Variable(torch.randn(input.size()[0], 100, 1, 1)) # We make a random input vector (noise) of the generator. Noise is vector os size 100. First arguement in randn is input.size()[0], 2nd arguement is number of elements in vector, 3rd and 4th arguement is just to give some fake dimensions that correspond to the feature map.Its like we have 100 feature map sof size 1x1. So we wrap this variable inside a Variable to make it compatible with torch variables as neural networks in pytorch only accept torch variables
            fake = netG(noise) # We forward propagate this random input vector into the neural network of the generator to get some fake generated images. fake is the mini batch of fake images. So we input a minibatch of random vectors and get minibatch of same size as output containing images
            target = Variable(torch.zeros(input.size()[0])) # We get the target. This time, we want to train discriminator to recognize the fake images, so we set target should be rejection of images, so correspond to 0, so we use zeros function
            output = netD(fake.detach()) # We forward propagate the fake generated images into the neural network of the discriminator to get the prediction (a value between 0 and 1). We save up memory as 'fake' contains gradients also, so we detach the gradient from the tocrh variable fake
            ...
            optimizerD.step() # We apply the optimizer to update the weights according to how much they are responsible for the loss error of the discriminator.We apply stochastic gradient descent to update the wights
            
            # 2nd Step: Updating the weights of the neural network of the generator
    
            netG.zero_grad() # We initialize to 0 the gradients of the generator with respect to the weights.
            #Next, we should get the input, but we already have the input, which is fake image(or minibatch of fake images) that are going to be th input of the discriminator
            target = Variable(torch.ones(input.size()[0])) # We get the target.this time we want to push the predictions to 1 as we want the discriminator to accept the fake images as real images. This is already wrappedin torch variables
            output = netD(fake) # We forward propagate the fake generated images into the neural network of the discriminator to get the prediction (a value between 0 and 1).In previous outputs, we detached the gradient of 'fake' to save memory. But this time we keep the grdient as we want to updat the wights of the generator, and to update them we will need gradient of 'fake'
            ...
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f' % (epoch, 25, i, len(dataloader), errD.data, errG.data)) # We print les losses of the discriminator (Loss_D) and the generator (Loss_G).We remove [0] of errD.data and errG.data as that was supported in an older version of pytorch
            if i % 100 == 0: # Every 100 steps:
                vutils.save_image(real, '%s/real_samples.png' % "./results", normalize = True) # We save the real images of the minibatch. save_image function saves a given tensor into an image file
            ...
            #If broken pipe error is coming or pickle error is coming add this line before the epochs start. if __name__ == "__main__":  